...
def sum_of_even_numbers(n: int) -> int:
    ...
    """
    return sum(filter(lambda x: x % 2 == 0, list(range(0, n + 1))))
...
def sum_of_multiples(n: int, end: int) -> int:
    ...
    """
    return sum(filter(lambda x: x % n == 0, list(range(0, end + 1))))
...
def remove_vowels(original_string: str) -> str:
    ...
    """
    vowels = "aeiouAEIOUõäöüÕÄÖÜ"
    return "".join(filter(lambda x: x not in vowels, original_string))
