...
def create_tuple_from_two_numbers(a: int, b: int) -> tuple:
    ...
    """
    if a == b:
        tuple_from_numbers = a,
    else:
        tuple_from_numbers = a, b
    return tuple_from_numbers
...
def remove_odd_numbers(numbers: tuple) -> tuple:
    ...
    for i in numbers:
        if i % 2 == 0:
            not_odd_numbers.append(i)
    return tuple(not_odd_numbers)
...
def insert_tuple_inside_tuple(outer_tuple: tuple, position: int, inner_tuple: tuple) -> tuple:
    ...
    """
    outer_tuple_list = list(outer_tuple)
    inner_tuple_list = list(inner_tuple)
    for i in range(len(inner_tuple)):
        outer_tuple_list.insert(position, inner_tuple_list.pop())
    return tuple(outer_tuple_list)