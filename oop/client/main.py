...
def read_from_file_into_list(filename: str) -> list:
    ...
    with open(filename, "r") as file:
        ...
        for line in file:
            ...
            clients.append(Client(line[0], line[1], int(line[2]), int(line[3]), int(line[4])))
        return clients
...
def filter_by_bank(filename: str, bank: str) -> list:
    ...
    with open(filename, "r") as file:
        ...
        for line in file:
            line = line.strip().split(",")
            clients.append(Client(line[0], line[1], int(line[2]), int(line[3]), int(line[4])))
        return [el for el in clients if el.bank == bank]
...
def largest_earnings_per_day(filename: str) -> Optional[Client]:
    ...
    with open(filename, "r") as file:
        ...
        for line in file:
            line = line.strip().split(",")
            clients.append(Client(line[0], line[1], int(line[2]), int(line[3]), int(line[4])))
        clients.sort(key=lambda client: client.earnings_per_day(), reverse=True)
        if len(clients) > 0:
            print(len(clients) > 0)
            if clients[0].starting_amount >= clients[0].current_amount:
                print(clients[0].starting_amount >= clients[0].current_amount)
                return None
            elif clients[0].earnings_per_day() == clients[1].earnings_per_day():
                if clients[0].account_age < clients[1].account_age:
                    return clients[0]
                else:
                    return clients[1]
            else:
...
def largest_loss_per_day(filename: str) -> Optional[Client]:
    ...
    with open(filename, "r") as file:
        ...
        for line in file:
            line = line.strip().split(",")
            clients.append(Client(line[0], line[1], int(line[2]), int(line[3]), int(line[4])))
        clients.sort(key=lambda client: client.earnings_per_day())
        if len(clients) > 0:
            if clients[0].starting_amount <= clients[0].current_amount:
                return None
            elif clients[0].earnings_per_day() == clients[1].earnings_per_day():
                clients.sort(key=lambda client: client.account_age)
                for i in clients:
                    print(i.name, i.earnings_per_day())
                if clients[0].account_age < clients[1].account_age:
                    return clients[0]
                else:
                    return clients[1]
